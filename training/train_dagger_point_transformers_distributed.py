...
def sampling(config, beta=1.0, num_checkpoint=0, num_workers=3, logger_dir=None, aug_data_dir=None, benchmark_config=None, bgtraffic=0, resample_config='random_uniform', num_config=3):
    ...
    call = 'python3.7 AutoCastSim/ScenarioRunner/scenario_runner_distributed.py \
            --file --sharing \
            --outputDir {}\
            --route AutoCastSim/ScenarioRunner/srunner/examples/DistributedMultiEgoVehicle.xml'

    AgentWrapperOne = 'python3.7 AutoCastSim/ScenarioRunner/agent_wrapper_main.py \
            --route AutoCastSim/ScenarioRunner/srunner/examples/DistributedMultiEgoVehicle.xml \
            --route_id 0'
    
    AgentWrapperTwo = 'python3.7 AutoCastSim/ScenarioRunner/agent_wrapper_main.py \
            --route AutoCastSim/ScenarioRunner/srunner/examples/DistributedMultiEgoVehicle.xml \
            --route_id 1'

    clean_process()
    p1=subprocess.Popen(call,shell=True)
    p1Agent1=subprocess.Popen(AgentWrapperOne,shell=True)
    p1Agent2=subprocess.Popen(AgentWrapperTwo,shell=True)
    ...
    p2AgentOne=subprocess.Popen(call,shell=True)
    p2AgentTwo=subprocess.Popen(call,shell=True)


    p1.communicate()
    p1Agent1.communicate()
    p1Agent2.communicate()

    p2.communicate()
    p2AgentOne.communicate()
    p2AgentTwo.communicate()
    clean_process()
